    ...
    <div className="min-h-screen flex items-center justify-center bg-black px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full px-4 py-2 bg-white text-black font-semibold rounded-lg hover:bg-zinc-100 disabled:bg-zinc-600 disabled:cursor-not-allowed">{loading ? 'Logging in...' : 'Log in'}</button>
        </form>
        <p className="text-center text-zinc-400 mt-6">Don&apos;t have an account? <Link href="/signup" className="text-white hover:text-zinc-300">Sign up</Link></p>
      </div>
    </div>
  )
}
